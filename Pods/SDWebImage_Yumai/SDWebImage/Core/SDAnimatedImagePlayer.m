...
@property (weak, nonatomic,readwrite) id<SDAnimatedImageViewDelegate> _Nullable delegate;
...
- (instancetype)initWithProvider:(id<SDAnimatedImageProvider>)provider delegate:(id)delegate {
    self = [super init];
    if (self) {
        NSUInteger animatedImageFrameCount = provider.animatedImageFrameCount;
        // Check the frame count
...
+ (instancetype)playerWithProvider:(id<SDAnimatedImageProvider>)provider delegate:(id)delegate {
    SDAnimatedImagePlayer *player = [[SDAnimatedImagePlayer alloc] initWithProvider:provider delegate:delegate];
    return player;
}
...
//        // Update the loop count when last frame rendered
        if (nextFrameIndex == 0) {
            ...
            UIImage *frame = [self.animatedProvider animatedImageFrameAtIndex:self.currentFrameIndex];

            NSUInteger maxLoopCount = self.totalLoopCount;
            if (maxLoopCount != 0 && (self.currentLoopCount >= maxLoopCount)) {

                if (_delegate && [_delegate respondsToSelector:@selector(animetionEnded:)]) {
                    [_delegate animetionEnded:maxLoopCount];
                    [self stopCustomPlaying];
                } else {
                    [self stopPlaying];
                }
...
//        if (_delegate && [_delegate respondsToSelector:@selector(animetionShouldEndedAtSpecifiedFrame)]) {
//            NSUInteger endedFrameIndex = [_delegate animetionShouldEndedAtSpecifiedFrame];
//            if (endedFrameIndex <= self.totalFrameCount && (endedFrameIndex == nextFrameIndex + 1)) {
//                [self stopPlaying];
//
//                if (_delegate && [_delegate respondsToSelector:@selector(animetionEndedAtFrameIndex:)]) {
//                    [_delegate animetionEndedAtFrameIndex:endedFrameIndex];
//                }
//                return;
//            }
//        }
